...
locals {
  values = {
    ...
    txtOwnerId = "example-idp1-dev.us-east1"
    serviceAccount = {
      annotations = {
...
resource "google_service_account" "external_dns" {
  account_id   = replace(substr("example-idp1-dev-external-dns", 0, 30), "/[^a-z0-9]$/", "")
  display_name = substr("ExternalDNS service account for k8s cluster: example-idp1-dev", 0, 100)
}
