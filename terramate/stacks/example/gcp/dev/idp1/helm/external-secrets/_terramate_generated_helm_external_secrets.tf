...
resource "google_service_account" "external_secrets" {
  account_id   = replace(substr("example-idp1-dev-external-secrets", 0, 30), "/[^a-z0-9]$/", "")
  display_name = substr("External Secrets service account for k8s cluster: example-idp1-dev", 0, 100)
}
